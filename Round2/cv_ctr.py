...
complete_df = pd.read_csv('comp_prefix_df.csv')
complete_df = complete_df.drop(columns=['title', 'is_in_title_c', 'leven_distance_c',
       'distance_rate_c', 'prefix_w2v_c'])

df_data = pd.concat([df_data,complete_df],axis=1)
...
cvstor = CrossFeatCVRCVStatistic(cvr_features,'label')
with utils.timer('CVR'):
    df_train = cvstor.triple_cvs(df_train)
...
#--------------------------------------
cvr_features = ['complete_prefix']
cvstor=CVRCVStatistic(cvr_features,'label')
with utils.timer('CVR'):
    df_train = cvstor.cvs(df_train)

cvr_features = [('complete_prefix','title'), ('complete_prefix','tag')]
cvstor=CrossFeatCVRCVStatistic(cvr_features,'label')
with utils.timer('CVR'):
    df_train = cvstor.cvs(df_train)

features = ['complete_prefix']
cvstor=HotCVStatistic(features,'label')
with utils.timer('Hot'):
    df_train = cvstor.cvs(df_train)
...
cvr_features = [('complete_prefix','title','tag')]
cvstor = CrossFeatCVRCVStatistic(cvr_features,'label')
with utils.timer('CVR'):
    df_train = cvstor.triple_cvs(df_train)
...
df_data.to_csv('cvr_cv_df_3.csv',index=False)
...
#cvr_feature = pd.read_csv('cvr_cv_feature_v2.csv')
...
print("processed data shape:",df_data.shape)
...
with utils.timer('fillna'):
    ...
    'prefix_w2v','max_similar','mean_similar','weight_similar','prefix_click', 'prefix_count','title_click','title_count','tag_click', 'tag_count',
    'prefix_title_click', 'prefix_title_count','prefix_tag_click', 'prefix_tag_count','title_tag_click', 'title_tag_count']
    for col in features:
...
#features+=['prefix_click', 'prefix_count','title_click','title_count','tag_click', 'tag_count',
#    'prefix_title_click', 'prefix_title_count','prefix_tag_click', 'prefix_tag_count','title_tag_click', 'title_tag_count']
...
# save_dir = '../xtyfile/submit/'
# os.makedirs(save_dir,exist_ok=True)
# with open(save_dir+'lgb_ml_bl_1023_'+str(thr)+'.csv','w') as fout:
#     for pred in y_pred:
#         fout.write(str(pred)+'\n')
y_pred = pd.DataFrame(y_pred)
y_pred.to_csv('xtyfile/submit/predict_3.csv',index=False,header=False)

a = zipfile.ZipFile('xtyfile/submit/test_3.zip','w')
a.write("xtyfile/submit/predict_3.csv")
a.close()
"""