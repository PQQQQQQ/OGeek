...
stat_df = stat_df.drop(columns = ['tag', 'label', 'max_query_ratio',
       'prefix_word_num', 'title_word_num', 'title_len',
       'small_query_num', 'query_length', 'prob_sum', 'prob_max',
       'prob_mean'])
...
class CompletePrefixProcessing(object):
    @staticmethod
    ...
    def _distince_rate(item):
        str1 = item["complete_prefix"]
        str2 = item["title"]
        leven_distance = item["leven_distance_c"]
    ...
    def get_prefix_df(self, df):
        ...
        prefix_df[["complete_prefix", "title"]] = df[["complete_prefix", "title"]]
        prefix_df["is_in_title_c"] = prefix_df.apply(self._is_in_title, axis=1)
        prefix_df["leven_distance_c"] = prefix_df.apply(self._levenshtein_distance, axis=1)
        prefix_df["distance_rate_c"] = prefix_df.apply(self._distince_rate, axis=1)
        prefix_df["prefix_w2v_c"] = prefix_df.apply(self._get_prefix_w2v, axis=1)
        return prefix_df
...
prefix_processing = CompletePrefixProcessing()
prefix_df = prefix_processing.get_prefix_df(df)

prefix_df.to_csv('comp_prefix_df_x.csv', index=False)
